...
# def modified_control_loop(agent, initial_state, data_df, desired_pH, control_keys, target_key, ph_model, poly_gbr_model, state_scaler, poly_gbr_scaler, poly):
...
# def main():
#     st.title("pH Control Simulation")

#     # Model selection
#     st.sidebar.header("Model Selection")
#     ph_model_file = st.sidebar.file_uploader("Upload pH prediction model (H5 file)", type="pkl")
...
#             "Select control keys (e.g., Fällungslauge.TotalVolume, Metallsulfatlösung.TotalVolume)",
#             all_columns,
#             default=["Fällungslauge.TotalVolume", "Metallsulfatlösung.TotalVolume"]
#         )
...
#             # Initialize agent
#             agent = DQNAgent(input_dim=12, output_dim=25)  # +1 for the target key

#             # Prepare initial state
#             initial_state = data_df.iloc[start_index][control_keys + [target_key]].to_dict()

#             # Run the modified control loop
#             results = modified_control_loop(agent, initial_state, data_df.iloc[start_index:end_index], desired_pH, 
